...
import com.enn.vo.energy.app.RealDataResp;
...
@RestController
@RequestMapping("/elecView")
@Api(tags = {"用电总览"})
public class ElectricityOverviewController extends BaseController{
    @Autowired
    ...
    @ApiOperation(value = "用电总览-历史运行数据", notes = "日、月、年历史数据")
     ...
     * /index/realData
     */
    @ApiOperation(value = "用电总览-运行实时数据", notes = "实时数据")
    @RequestMapping(value = "/realData", method = RequestMethod.POST)
    public EnergyResp<RealDataResp> getRealData(@RequestBody @Valid CustReq custReq, BindingResult result) {
        EnergyResp<RealDataResp> energyResp = new EnergyResp<RealDataResp>();
        if (result.hasErrors()) {
            energyResp.faile(StatusCode.C.getCode(), StatusCode.C.getMsg(), result.getFieldError().getDefaultMessage());
            return energyResp;
        }
        ...
        if (StatusCode.SUCCESS.getCode().equals(listEnergyResp.getCode())) {
            ...
            //获取有功功率
            LastStaReq sa = new LastStaReq();
            sa.setMetric("EMS.P");
            sa.setEquips(equips);
            EnergyResp<ListResp<LastResp>> p = opentsdbService.getLast(sa);
            if(StatusCode.SUCCESS.getCode().equals(p.getCode())){
//                realDataResp.setActivePower(new BigDecimal(p.getData().getList().get(0).getValue()).toString());
            }


            //获取无功功率
            sa.setMetric("EMS.Q");
            EnergyResp<ListResp<LastResp>> q = opentsdbService.getLast(sa);
//            realDataResp.setReactivePower(new BigDecimal(p.getData().getList().get(0).getValue()).toString());

            //本月的平均功率因数
            sa.setMetric("EMS.COS");
            EnergyResp<ListResp<LastResp>> cos = opentsdbService.getLast(sa);
//            realDataResp.setReactivePower(new BigDecimal(p.getData().getList().get(0).getValue()).toString());

            //获取本月的需量最大值，以及发生的时间
            Date lastOneDay = DateUtil.getOneDay();
            String start = DateUtil.formatDate(lastOneDay, "yyyy-MM-dd HH:mm:ss");
            String end = DateUtil.formatDate(new Date(), "yyyy-MM-dd HH:mm:ss");
            SamplDataStaReq staReq = new SamplDataStaReq();
            staReq.setDownsample(Constant.DOWNSMPLE_1M);
            staReq.setEquips(equips);
            staReq.setMetric("EMS.P");
            staReq.setStart(start);
            staReq.setEnd(end);
            EnergyResp<ListResp<RmiSamplDataResp>> max = opentsdbService.getSamplDataStaReq(staReq);
//            Date add1 = DateUtils.add(DateUtil.parseTime(max.getTime()));
//            String en = DateUtil.formatDate(add1, "HH:mm");
//            String st = DateUtil.formatDate(DateUtil.parseTime(max.getTime()), "MM-dd HH:mm");
//            realDataResp.setMaxTime(st + "-" + en);
//            realDataResp.setAveragePower(MathUtils.towDecimal(max.getValue()).toString());
//            realDataResp.setUpdataTime(p.getData().getTime());
//            energyResp.ok(realDataResp);
        } else {
            energyResp.faile(StatusCode.E_B.getCode(), StatusCode.E_B.getMsg(), "该用户下无电表！！！");
        }
        return energyResp;
     ...
     * /index/realData
     */
    @ApiOperation(value = "用电总览-厂连续15日用电趋势", notes = "用电总览-厂连续15日用电趋势")
